...
if __name__ == "__main__":
    ...
    test_learning_rate = [0.0003,0.001,0.003,0.01,0.03,0.1,0.3,1]
    acc_compare = {}
    for lr in test_learning_rate:
        print("The learning rate is {} ".format(lr))
        parameters, best_acc = model(X_train.T, Y_train.T, X_dev.T, Y_dev.T, args['file_name'], num_iterations=int(args['num_iter']), learning_rate=lr)
        acc_compare[lr] = best_acc
    print(acc_compare)
    '''
    ...
    print('\n[Start evaluating on the official test set and dump as {}...]'.format(args['file_name'] + '.csv'))
    revs, _ = data_preprocess("./twitter-sentiment.csv", args['clean'], int(args['max_vocab']))
    test_data, _ = feature_extraction_bow(revs, word2idx)
    write_testset_prediction(parameters, test_data.T, args['file_name'] + '.csv')
    '''